...
import scriptella.core.ThreadLocalContext;
...
public class EtlExecutorBean extends EtlExecutor implements InitializingBean, BeanFactoryAware {
    private static final String BEAN_FACTORY_CONTEXT_KEY =
            EtlExecutorBean.class.getName() + ".beanFactory";
     ...
     */
    private static void setContextBeanFactory(BeanFactory f) {
        ThreadLocalContext.set(BEAN_FACTORY_CONTEXT_KEY, f);
    }
    ...
    static BeanFactory getContextBeanFactory() {
        BeanFactory f = (BeanFactory) ThreadLocalContext.get(BEAN_FACTORY_CONTEXT_KEY);
        if (f == null) {
