...
public class Driver extends ScriptellaJDBCDriver {
    ...
    protected java.sql.Connection getConnection(String url, Properties props) throws SQLException {
        if (url == null) {
            throw new JDBCException("JNDI name must be specified in an url attribute of connection element.");
        }
        try {
            ...
            throw new JDBCException("A problem occured while trying to lookup a datasource with name " + url, e);
        }
